...
const getNotificationTypeMeta = (type: Notification['type']) => {
  switch (type) {
    case 'like':
      return { label: 'Thông báo về bài viết của bạn', icon: <Heart size={16} className="text-white" />, iconWrap: 'bg-rose-500' }
    case 'comment':
      return { label: 'Bình luận mới', icon: <MessageCircle size={16} className="text-white" />, iconWrap: 'bg-blue-500' }
    case 'share':
      return { label: 'Chia sẻ nội dung', icon: <Share2 size={16} className="text-white" />, iconWrap: 'bg-emerald-500' }
    case 'follow':
      return { label: 'Theo dõi mới', icon: <UserPlus size={16} className="text-white" />, iconWrap: 'bg-violet-500' }
    case 'unfollow':
      return { label: 'Bỏ theo dõi', icon: <UserMinus size={16} className="text-white" />, iconWrap: 'bg-slate-500' }
    case 'announcement':
      return { label: 'Thông báo hệ thống', icon: <Megaphone size={16} className="text-white" />, iconWrap: 'bg-orange-500' }
    default:
      return { label: 'Thông báo', icon: <Bell size={16} className="text-white" />, iconWrap: 'bg-orange-500' }
  }
}

// ── Video preview: hiển thị paused, click để play/pause ─────────────────────
function ReelVideoPreview({ reel }: { reel: Reel }) {
  ...
  return (
    <div
      className="mt-3 relative overflow-hidden rounded-xl aspect-video bg-black cursor-pointer group"
    ...
    >
      ...
      {/* Play/Pause overlay — ẩn khi đang play, hiện lại khi hover */}
      ...
      <div className="absolute bottom-0 left-0 right-0 p-3 bg-gradient-to-t from-black/60 to-transparent pointer-events-none">
        <div className="flex items-center gap-3 text-white text-xs">
          <span className="flex items-center gap-1"><Heart size={12} className="fill-white" /> {reel._count?.likes ?? 0}</span>
          <span className="flex items-center gap-1"><MessageCircle size={12} /> {reel._count?.comments ?? 0}</span>
        </div>
      </div>
    </div>
  )
        ...
        <section className="rounded-3xl border border-orange-100 bg-white p-5 md:p-7 shadow-sm">
          ...
          {notification.type === 'announcement' ? (
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-2xl border border-orange-100 bg-orange-50/40 p-4">
                <div className="flex items-center gap-3">
                  {notification.actorAvatar ? (
                    <Avatar src={notification.actorAvatar} name={notification.actorName || 'U'} size="xl" className="w-12 h-12" />
                  ) : (
              ...
              {relatedPost && (
                <article className="rounded-2xl border border-orange-100 bg-white p-4">
                  <div className="flex items-center justify-between">
                    ...
                    >
                      Xem bài viết <ChevronRight size={13} />
                    </Link>
                  </div>
              ...
              {/* Related reel — VIDEO player thay vì img */}
              {relatedReel && (
                <article className="rounded-2xl border border-orange-100 bg-white p-4">
                  <div className="flex items-center justify-between">
