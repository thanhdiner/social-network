...
export class FriendsController {
  ...
  getReceived(
    ...
    return this.friendsService.getReceivedRequests(req.user.id, page, limit);
  }
  ...
  getSuggestions(
    ...
    return this.friendsService.getSuggestions(req.user.id, limit);
  }
  ...
  getFriends(
    ...
    return this.friendsService.getFriends(req.user.id, page, limit);
  }

  // GET /friends/:userId — danh sách bạn bè của user khác
  @Get('user/:userId')
  getUserFriends(
  ...
  cancelRequest(@Req() req, @Param('userId') receiverId: string) {
    return this.friendsService.cancelRequest(req.user.id, receiverId);
  }
  ...
  unfriend(@Req() req, @Param('userId') targetUserId: string) {
    return this.friendsService.unfriend(req.user.id, targetUserId);
  }
}
